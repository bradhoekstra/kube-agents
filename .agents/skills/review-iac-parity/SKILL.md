...
- **`imagePullSecrets` is expressible from the CRD, the chart, and Terraform only.** `spec.deployment.imagePullSecrets` and the operator's `IMAGE_PULL_SECRETS` default are reachable through `global.imagePullSecrets` and `image_pull_secrets`. Nothing on the script path is: not `vars.sh`, not `platform-agent.yaml.template`, not `provision_03`, and no `install.sh` flag alongside `--registry-prefix`, so a script install still needs a mirror the nodes can read on their own. New install configuration goes to the chart and Terraform; closing this on the scripts is a change in its own right, not silent drift. Nothing scalar for `make iac-parity-check` to compare — the chart and Terraform both default to empty and the scripts have no counterpart.
