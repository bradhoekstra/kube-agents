...
func TestPlatformAgentReconciler_Reconcile(t *testing.T) {
	...
	if !found {
		t.Errorf("expected Deployment to contain the agent-api-auth sidecar")
	} else if authC.RestartPolicy == nil || *authC.RestartPolicy != corev1.ContainerRestartPolicyAlways {
		t.Errorf("agent-api-auth must be a native sidecar (restartPolicy: Always) so it binds its ports before the agent container starts")
	}
