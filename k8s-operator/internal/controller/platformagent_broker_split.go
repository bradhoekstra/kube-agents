...
func validateCredentialBrokerSplit(agent *agentv1alpha1.PlatformAgent) (string, string) {
	...
	// The sandbox placement keeps the watcher in the agent Pod — see
	// buildAgentAPIAuthSidecar — and takes precedence over this flag, so there is
	// nothing to refuse.
...
// credentialBrokerOffPod reports whether the broker runs anywhere other than
// the agent's own Pod, by either of the two switches that move it.
//
// Everything that follows from being off the Pod follows from both: Envoy binds
// the Pod IP instead of loopback, the caller has to be authenticated because
// loopback is no longer the control, and the agent has to carry a token to
// present. Written as one predicate rather than repeated as an `||` so that a
// third placement -- or the abstraction making this unconditional -- changes
// one line and cannot leave a call site on the narrower test, which would show
// up as a broker refusing to boot rather than as a compile error.
func credentialBrokerOffPod(agent *agentv1alpha1.PlatformAgent) bool {
	return credentialBrokerIsSplit(agent) || shellSandboxEnabled(agent)
}
...
//
// Loopback only while the broker is still a sidecar of this Pod. Two switches
// take it off loopback and both land on the same Service name, because
// credentialProxyName and credentialBrokerName render the same string:
// splitCredentialBrokerPod puts the broker in a Deployment of its own, and the
// shell sandbox puts it in the sandbox's StatefulSet. Either way the gateway's
// relay clients dial a Service and should not have to know which.
