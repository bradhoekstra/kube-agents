...
func buildCredentialProxyContainer(agent *agentv1alpha1.PlatformAgent) corev1.Container {
	...
	return corev1.Container{
		...
		Resources: corev1.ResourceRequirements{
			...
			// in-flight request. Envoy is small and steady: 53 to 66Mi resident
			// on a live install, 85Mi in the report that led here. The broker's
			// own share is bounded: credential_proxy.py keeps at most
			// CREDENTIAL_PROXY_MAX_OUTPUT_BYTES of each stream, bounds the
			// decoded text to the same size (output that is not UTF-8 would
			// otherwise triple), holds about six times that per request at
			// peak (the two buffers, their decoded text, the JSON body and its
			// encoding; 48Mi measured against the 8 MiB cap, 37Mi for output
			// that is not UTF-8), and admits at most
			// CREDENTIAL_PROXY_MAX_CONCURRENT_COMMANDS requests at once, each
			// holding its slot until its response is written -- 384Mi at the
			// 8 MiB cap and the default of eight, which the cap test in
			// platformagent_manifests_test.go asserts against this limit. The
			// term that varies is the children: kubectl 1.35 listing 4,000 pods
			// measured 432Mi resident for -o json and 1,281Mi for -o yaml. On a
			// cgroup v2 node the kubelet arms the group OOM killer for the
			// container, so one such child past the limit takes Envoy and the
			// broker down with it -- a container restart, not one failed
			// command. An install whose clusters make that routine moves
