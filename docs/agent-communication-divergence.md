...
The harness now has a dedicated **front door**. The `default` Hermes profile is a thin **Chat Agent** (`agents/chat/`) that receives all chat ingress, discovers the available specialist agents and their responsibilities (via the `router` MCP tool `list_agents`), delegates the request over the asynchronous kanban board (`kanban_create`), and relays the progress and response. (An earlier revision also exposed a synchronous `router` `ask_agent` relay for quick lookups; it was removed in favor of a kanban-only model so every delegation is non-blocking and streams progress into the thread.) Today's Platform Agent is demoted to a named `platform` profile scaffolded at pod startup (`deploy/shared/docker-entrypoint.sh` + `scripts/profile_scaffold.py`) from `agents/platform/`. The pod/Deployment/CR names are unchanged.
