...
- **`incident_context`** — injects Kubernetes incident context into known chat threads on reply (`pre_gateway_dispatch` hook). The work happens on the Chat Agent, which enables it too: the pod runs a single gateway and it is homed at that profile, so an incident-thread reply is dispatched there and never here. It stands aside for a message that starts with `/`: `legacy_slash_commands` and `direct_agent_routing` are on the same hook, and prepending the triage report first would move the command off the front of the line where their anchored patterns can no longer see it.
