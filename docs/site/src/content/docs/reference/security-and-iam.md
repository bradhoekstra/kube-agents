...
- **No credentials in the sandbox.** API keys, chat tokens, and ServiceAccount tokens live only in the Envoy credential-proxy container; the agent container gets wrapper CLIs that forward to it through a policy-enforced proxy. See [Credential isolation](/kube-agents/reference/credential-isolation/).
  ...
  **This is a filesystem and environment boundary, not a network one, and the default install does not close it.** The proxy shares a Pod — and therefore a network namespace and one Pod identity — with the sandbox, so the agent container can reach the link-local metadata server at `169.254.169.254` and mint the Workload Identity service account's token itself, bypassing the proxy and every command policy in front of it. Three opt-in fields work on that path, `spec.security.splitCredentialBrokerPod`, `spec.security.egressPolicy: Allowlist` and `spec.harness.experimental.shellSandbox.enabled`, and **all default to off**. With either of the first two on, the `*-gateway-netpol` described below still permits the metadata path to the same Pod, because NetworkPolicies are additive; the shell sandbox closes it by identity instead, giving the sandbox Pod a ServiceAccount with no `iam.gke.io/gcp-service-account` annotation. What they cost, what they break, and what is still open are on [Credential isolation](/kube-agents/reference/credential-isolation/#denying-the-sandbox-the-metadata-server), which owns the topic.
