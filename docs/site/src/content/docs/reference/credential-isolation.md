...
**Limitation:** the credential runtime is in its own Pod, but both Pods still run under the same ServiceAccount, so the sandbox reaches the GKE metadata server and obtains the same Workload Identity the proxy uses. Moving the container was the reachability half of the change; taking the binding off the agent's ServiceAccount is the half that closes this, and it has not landed. [`designs/credential-proxy-placement.md`](https://github.com/gke-labs/kube-agents/blob/main/docs/designs/credential-proxy-placement.md) is canonical for what is and is not delivered. Envoy also authenticates no caller: loopback was the access control before the split, and what guards the Service now is a NetworkPolicy, which a cluster without a policy-enforcing CNI ignores.
