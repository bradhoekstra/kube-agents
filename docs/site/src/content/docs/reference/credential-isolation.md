...
Two settings move the broker off that Pod, and both make the same one change: the sandbox is given a projected ServiceAccount token so it can authenticate across the network. [`spec.security.splitCredentialBrokerPod`](#splitting-the-broker-into-its-own-pod) moves the broker alone; [`spec.harness.experimental.shellSandbox.enabled`](#running-the-shell-in-its-own-pod) moves the shell instead and takes the broker with it. Every unqualified "the sandbox holds no token" statement on this page describes the sidecar layout; [the agent now holds a credential](#the-agent-now-holds-a-credential-and-that-was-a-choice) has the trade.
...
| Explicit projected KSA token     | Not mounted in the sidecar layout; mounted read-only under either off-Pod layout | Read-only, one-hour token |
...
`spec.harness.experimental.shellSandbox.enabled` moves the shell rather than the broker. Everything the agent executes — the wrappers, `execute_code`, and the file tools — runs in a `<name>-shell` StatefulSet reached over SSH, and the credential runtime moves there as a sidecar of that Pod. The gateway Pod is left holding the model, the skills and the chat adapters, and no CLI at all. [`docs/designs/agent-shell-sandboxing.md`](https://github.com/gke-labs/kube-agents/blob/main/docs/designs/agent-shell-sandboxing.md) is canonical, including the one-time Workload Identity pool setup that no install surface performs.

It closes both halves of the limitation above, and for different reasons. The metadata server is closed by identity: the sandbox Pod's ServiceAccount carries no `iam.gke.io/gcp-service-account` annotation, so the metadata server answers every container in it with a principal IAM grants nothing, and the broker takes its cloud identity instead from a projected token mounted in its container alone and exchanged over Workload Identity Federation. Mounts are per-container where Pod identity is not.

The shared workspace is closed by removing it. With the sandbox on, `CREDENTIAL_PROXY_CONTENT_WORKSPACE` is set unconditionally and the skills that write to GitHub hand the broker file content and a commit message rather than a directory both containers mount, so there is no `.git/config` on a common volume for either side to write. There is no field to turn that off: an install that could would be choosing to keep the class open.

What it does not close is the same thing the split does not close. The two Pods still share a ServiceAccount for the Kubernetes API, and the token the sandbox presents to the broker is a file the model can read — the trade in [the agent now holds a credential](#the-agent-now-holds-a-credential-and-that-was-a-choice) applies here unchanged.

Two differences from the split are worth knowing before you compare them. The broker is a sidecar again, so the event watcher is not stranded: it stays in the gateway Pod inside `agent-api-auth`, with its own Kubernetes-API token, and `harness.eventWatcher.enabled` needs no change. And the broker's listener is on the Pod network rather than loopback, so it authenticates its callers exactly as the split broker does — `TokenReview` against an audience-bound projected token, `401` for anything else.
...
This is an availability problem rather than an authentication one — a rejected credential comes back as an HTTP error, not a refused connection. Which cause applies depends on where the broker is.
...
_Shell sandbox layout (`spec.harness.experimental.shellSandbox.enabled: true`)._ The broker is a sidecar again, of the sandbox Pod rather than the gateway, so this message means that Pod is unhealthy — and because the broker is a native sidecar, a broker that will not start keeps the whole sandbox Pod in `Init:CrashLoopBackOff` and the shell unreachable. The StatefulSet is the thing to look at, not the Deployment.

```bash
kubectl get pods -n kubeagents-system -l app=platform-agent-shell
kubectl logs -n kubeagents-system platform-agent-shell-0 -c envoy-credential-proxy
```
