...
**The broker owns the only checkout.** The agent no longer creates a directory the broker then runs `git` in, because there is no writable volume both can see (the one object both Pods project is the GitOps state ConfigMap, a read-only list of repository names). A skill that wants to change a repository sends file content and a commit message, and the broker writes, commits and pushes in a working tree on its own volume that the agent has no path to. That closes the `.git/config` class outright: `core.fsmonitor` and every other setting that names a program to run is read by `git` on every invocation, including read-only ones, and no argument-level policy could enumerate it — but the agent cannot write into the tree it is read from.
...
These are **deny** rules shipped in the operator's policy ConfigMap, not entries in the broker's built-in allowlist the paragraph below is about, so a false refusal here is fixed in a different place — see the appeal note at the end of this section.
...
A first deployment in a live environment will find read-only commands nobody anticipated. A `kubernetes.*` or `gcp.*` refusal means the broker's built-in allowlist lacks the verb, which is a change to kube-agents rather than to your install — except `gcp.scoped-sa.unmapped-scope`, which is not a verb problem at all: it means the request named a cluster the [scoped service account pool](/kube-agents/reference/security-and-iam/#the-scoped-service-account-pool) has no entry for, and the fix is adding the cluster to the pool's mapping (or leaving the pool disarmed). A `github.*` or `git.*` refusal comes from the deny policy the operator renders, which is a different review. Report the blocked command to your infrastructure team with the rule id from the refusal; the change stays scoped to the one command that was missing.
...
None of these is enforced by the operator. The broker always runs outside the agent Pod, so the field has no prerequisite to refuse.
...
There is no ordering to get wrong beyond those two steps: the broker cannot come back into the agent Pod.
...
**A proxied command fails with `the credential proxy is already running 8 commands and none finished within 60s; retry shortly`.** The broker runs a bounded number of commands at once (`CREDENTIAL_PROXY_MAX_CONCURRENT_COMMANDS`, 8 by default), because each one is a child process the container's memory limit has to hold. A request that cannot get a slot within a minute is refused with a `503` rather than run. The broker's log shows the queueing (`command waited … for a slot`) and the refusal (`command queued too long`). Retry, or raise the cap through `spec.deployment.env` together with the container's memory limit if the install routinely runs that many commands at the same time; long-running commands such as `kubectl logs --follow` or `kubectl wait` hold a slot for as long as they run.
