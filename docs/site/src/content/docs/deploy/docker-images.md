...
| `envoy` | `docker.io/envoyproxy/envoy` | `v1.39.0` | `ENVOY_IMAGE` | deploy/docker/Dockerfile (credential-proxy stage). |
...
There is no cluster or forge tooling in this image, in any form, and a build guard fails if any reappears. `kubectl`, `gcloud`, `gh`, `git`, `helm` and `yq` are all in the `agent-sandbox` image, which is where the agent's shell runs; so are the credential-proxy shims — symlinks to a client that forwards a command to the sidecar holding the credential — which used to stand in for the first four here. Agent-pod code that needs one of them reaches the sandbox over SSH through `agents/platform/scripts/sandbox_exec.py`. Until [#737](https://github.com/gke-labs/kube-agents/issues/737) Part C gives the credential proxy an address off the agent pod's loopback, the sandbox cannot reach it, and every credentialed command reports `CREDENTIAL_PROXY_URL is not configured` rather than running.
