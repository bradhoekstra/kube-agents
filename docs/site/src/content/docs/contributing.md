...
This pass is not the only one [`AGENTS.md`](https://github.com/gke-labs/kube-agents/blob/main/AGENTS.md) requires before a pull request opens: the docs-drift pass runs on every change, and the IaC-parity pass on any change crossing more than one install surface. In Claude Code, `/pr-preflight` covers all three — a subagent per applicable pass, one merged list back — and its plumbing lives in [`.agents/skills/review-preflight/SKILL.md`](https://github.com/gke-labs/kube-agents/blob/main/.agents/skills/review-preflight/SKILL.md) for any harness without slash commands. Reach for it rather than the session you are already in: coding agents are typically told not to spawn subagents unless asked, and invoking the command is that ask.
