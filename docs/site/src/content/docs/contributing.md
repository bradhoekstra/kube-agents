...
Every pull request is also reviewed by `kube-agents-bot`, a GitHub App that drives Claude Code over the branch diff. It only comments — it never pushes commits and never merges, and it does not replace the human review above.
