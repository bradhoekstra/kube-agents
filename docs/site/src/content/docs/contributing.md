...
- **Pushing more commits does not re-trigger it**, with one exception: a branch the bot last said does not merge gets its review after the next push. To ask for a fresh review of the current commit, comment `/review` on a line of its own — a strict read: what the bot is certain of, plus any high-severity finding just under that bar, marked as such — or `/review all` for one as wide as its first review. Owners, members, and collaborators can trigger it. A re-read takes about as long as the first.
