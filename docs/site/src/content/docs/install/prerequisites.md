...
Both commands omit a key whose value is unset, so a requirement that is not met usually shows as a missing key rather than a `false`. `autopilot: {}` means a Standard cluster; `autopilot: {enabled: true}` means Autopilot.
