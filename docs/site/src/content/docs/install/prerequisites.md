...
The shipping install path targets GKE. You'll need one working GCP project plus the standard command-line tools, and cert-manager installed on the cluster so the operator's admission webhooks come up cleanly. If the cluster already exists, it also has to meet the [cluster requirements](#cluster-requirements) below.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **GKE 1.29 or newer.** The chart's `kubeVersion` floor, for native sidecar containers. The optional `ValidatingAdmissionPolicy` needs 1.30, `AgentPlugin` OCI volumes 1.35.                                                                                  | `currentMasterVersion`                                                                                          | Nothing checks it up front. The Helm release fails after the GCP resources exist.                                                                                                                                                                                                                                                                                         |
...
| **NetworkPolicy enforcement**: Dataplane V2, or the legacy Calico addon. Every NetworkPolicy the install ships is accepted and inert without one; GKE Standard's default has neither.                                                                        | `datapathProvider: ADVANCED_DATAPATH`, or `networkPolicy.enabled: true`                                         | `install.sh` enables the Calico addon and then enforcement, which can recreate node pools. A bare Terraform run refuses the plan. The `FQDNNetworkPolicy` companions only enforce on Dataplane V2 with FQDN policy enabled, so prefer it; Calico leaves them inert.                                                                                                       |
| **A control plane reachable from where Terraform runs and from the agent**: a public IP endpoint, or the DNS endpoint with external traffic allowed. Terraform's Helm provider always uses the IP endpoint.                                                  | `enablePrivateEndpoint` unset or `false`, or `dnsEndpointConfig.allowExternalTraffic: true`                     | Nothing checks it; the Helm provider fails to connect. The module's `allow_external_dns_traffic` only manages clusters it created, so set it here with `gcloud container clusters update --enable-dns-access`. See [`gke_dns_endpoint.sh`](https://github.com/gke-labs/kube-agents/blob/main/scripts/installer/gke_dns_endpoint.sh) for why the flag is not passed blind. |
| **cert-manager**, present or absent, but declared either way.                                                                                                                                                                                                | `kubectl get deployment cert-manager -n cert-manager`                                                           | `install.sh` probes for it and sets `enable_cert_manager` for you; a bare Terraform run against a cluster that already has it fails on the existing CRDs unless you set it to `false`. Details [below](#cert-manager-on-the-target-cluster).                                                                                                                              |
| **A gVisor RuntimeClass**, because the agent runs sandboxed by default (`--gvisor=false` opts out).                                                                                                                                                          | Autopilot: `currentMasterVersion` at or above `1.27.4-gke.800`. Standard: nothing to read; the install adds it. | `install.sh` refuses an older Autopilot cluster before applying anything. On Standard the composition adds a `gvisor-pool` node pool of one `e2-standard-4` per zone: new billable capacity, and it carries GKE's sandbox taint, so only the sandboxed agent lands there.                                                                                                 |
| **Schedulable capacity for the workloads the chart adds.** A stock install requests about 1.5 vCPU and 4 GiB in total, and the agent pod alone needs about 1.25 vCPU free on one node. Hindsight memory, the dashboard, and the GitHub minter each add more. | `kubectl describe nodes` allocatable minus requests, on the pools the agent can use                             | Nothing checks it. The Helm release waits ten minutes on Pending pods and fails with `context deadline exceeded`, naming nothing.                                                                                                                                                                                                                                         |
...
Run the installer with `--dry-run` to see the plan against the cluster without changing anything.
