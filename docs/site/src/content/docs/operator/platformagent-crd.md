...
- `sharedVpcHosts` and `metricsScopes` — project IDs of Shared VPC host projects, and of the scoping projects of Cloud Monitoring Metrics Scopes. Neither is a Resource Manager container. Each is resolved on every run to the projects it reaches (`gcloud compute shared-vpc list-associated-resources <host>` for the service projects attached to a host, the host itself not among them, so name it in `projects` if its own clusters are wanted; `gcloud beta monitoring metrics-scopes describe locations/global/metricsScopes/<id>` for the projects a scope monitors, the scoping project included), and those projects then take the `projects` path above: each lists its own clusters, needs the read roles granted in it by hand until the Terraform composition gains a scope input, and reads `denied`, `api-disabled` or `unreachable` on its own. Nothing is inherited through a Shared VPC or a Metrics Scope, so a project attached or linked after install reads `denied` until it is granted. The lookups themselves need `compute.projects.get` in the host project (`roles/compute.viewer` carries it) and `resourcemanager.projects.get` in the scoping project (every read role above carries it). The Monitoring API names monitored projects by project number, so the run names each with `gcloud projects describe <number>`; a monitored project the account cannot read cannot be named either, and is reported by number as `denied`, or under its ID once an earlier run has named it (its snapshot row keeps the number). A project that is not a Shared VPC host resolves to no members. A selector's projects fill the set after the explicit projects and before the containers, each reading `over-cap` on its own past the cap. A selector the run cannot read is frozen exactly as a container is: the members the previous snapshot reached through it are carried under that outcome, nothing is created under them, and the scope prune stays off for the run; the `containers` array carries a row for it under its `via` name (`sharedVpcHosts/<host>`, `metricsScopes/<id>`). There is no index and no lag behind either lookup: a project detached from the host or unlinked from the scope leaves the set on that run and retires over the ordinary two clean runs. A rollback to an operator that predates these fields keeps their members, as for containers.
...
the callout ordering gate becomes visible: while `BusCredentialsReady` holds a first gateway
creation back, the phase reads `Provisioning` and names the Deployment it is waiting on. So to gate
