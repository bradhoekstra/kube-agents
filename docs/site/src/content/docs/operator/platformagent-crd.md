...
exactly as it is), so record it in `install.env` first; a full run refuses while the CR names
projects and `SCOPE_PROJECTS` names none, and prints the lines to add. A key that names any project is
the declaration and changes the scope freely, in full mode.
