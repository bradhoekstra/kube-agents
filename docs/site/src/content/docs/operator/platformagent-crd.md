...
That file also pins four values that are not credentials at all. The first is `API_SERVER_KEY=cluster-internal-trusted`,
...
the reconcile a declaration the agent authored. Its one reader is
