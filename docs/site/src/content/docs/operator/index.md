...
- A `PodDisruptionBudget` selecting the Deployment's pods. At the default single replica it is `maxUnavailable: 1`, which declares the agent evictable rather than blocking node drains; raise `spec.deployment.availability.replicas` above 1 and it becomes `minAvailable: 1` instead.
