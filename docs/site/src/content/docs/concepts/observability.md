...
**Nothing in the `envoy-credential-proxy` container may log credential material.** That container, in the credential-proxy Pod, is the one place holding cluster credentials, GCP tokens, and chat secrets, and everything it writes to stdout leaves the cluster through the path above. The same rule covers the event watcher, which runs in the gateway Pod's `agent-api-auth` sidecar: it logs identifiers — cluster, namespace, pod, event reason, profile directory — and never a token, a kubeconfig body, or a request header.
