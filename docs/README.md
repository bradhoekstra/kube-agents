...
| `docs/designs/drift-detection.md` | Feature design | Design of record, partially implemented. Splits drift into computing the diff (commoditized) and judging it (the differentiator); enters the shipped pipeline as a `gitops-drift` inject. | `managedFields` + audit-log attribution, revert-or-codify, CUJ selection | Sink and audit ingestion exist (`drift-pubsub`, `drift-detector`); nothing classifies or injects yet (banner in file) |
...
| `k8s-operator/cmd/drift-detector/README.md` | Component README | The Go daemon that pulls GKE audit records off the `drift-pubsub` subscription and parses them into the attributed changes drift judgement is built on. | Audit ingestion, three-way settling, `resourceName` grammar | Drift-detector developers |
