...
| `agents/chat/defaults/plugins/direct_agent_routing/README.md` | Design doc | How `/platform <text>` reaches the Platform Agent's own profile with no Chat Agent turn and no kanban card: the `pre_gateway_dispatch` stamp on `event.source.profile`, why it rewrites rather than skips (authorization still runs), what falls through unrouted, and what a routed turn gives up. | Direct routing, `pre_gateway_dispatch`, `multiplex_profiles`, roster resolution | Human design doc for contributors changing the flow |
