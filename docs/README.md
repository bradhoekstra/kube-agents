...
| `docs/designs/agent-shell-sandboxing.md` | Feature design | Design of record, not yet implemented. Moves the agent's shell out of the agent pod into an Agent Sandbox reached over Hermes' `ssh` backend, and puts the Session KV behind its own interface; traces a `terminal` call end to end and states what breaks first. | Terminal backend survey, Agent Sandbox vs Agent Substrate, `sync_back` write channel, gVisor/WAL blocker | Status banner declares it unimplemented; #737 Parts A and B |
...
| `docs/designs/credential-proxy-placement.md` | Feature design | Design of record, not yet implemented. Why the credential proxy cannot hold its confidentiality property as a sidecar — Workload Identity is scoped to the pod, not the container — and moves it to a dedicated Deployment per agent following the `github-token-minter` shape. | Metadata-server path, possession vs use, five fused roles, reversed relay URLs | Status banner declares it unimplemented; #737 Part C |
