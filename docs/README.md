...
| `terraform/modules/kube-agents-iam/README.md` | Component README | Reusable Terraform module for provisioning the agent's GSA, Workload Identity binding, read-only IAM role set, the scoped service account pool, and the read grants in the projects `scope` names. | GCP IAM, Workload Identity, role grants | Infrastructure engineers |
