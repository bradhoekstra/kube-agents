...
| `docs/designs/agent-shell-sandboxing.md` | Feature design | The whole of #737: moves the agent's shell out of the agent pod into a per-agent sandbox StatefulSet reached over Hermes' `ssh` backend, and moves the credential proxy in beside it on an unbound ServiceAccount so no container in that Pod can mint the agent's GCP identity. | Terminal backend survey, Agent Sandbox vs Agent Substrate, pod-scoped Workload Identity, federation and the ID-token gap, `sync_back` write channel, gVisor/WAL blocker | Shell sandbox and proxy co-location implemented; #737 Part A (Session KV interface) not |
...
| `docs/designs/testing-strategy.md` | Feature design | What "tested" means for an autonomous actor with standing authority in a production fleet, and the four tiers that answer it: unit, integration (real seams, fake agent), presubmit evals, release gate, with nightly named but not built. Carries the domain-coverage rule, the six-rung eval gate ladder, and the statistical treatment of judged scores. | Tiers, domain as unit of coverage, 3 repetitions per case, non-inferiority against `main`'s baseline, pinned judge and pinned scorer, keep writing new cases | **Draft**, in review; §4.2's statistical gate is advisory until baselines exist. Unit tests are real and the integration tier gates; the seeded fleet is applied and standing; presubmit runs two cases and blocks nothing; the release gate is one test; nightly is deferred (§4.4) |
