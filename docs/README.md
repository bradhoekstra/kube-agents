...
| `docs/designs/multi-project-scope.md` | Feature design | Why the Platform Agent manages one GCP project today (one `clusters list`, one project of IAM bindings, "1 per project" in the architecture) and the opt-in scope that replaces it: explicit projects, folders, organisations, Shared VPC hosts and Metrics Scopes on `spec.scope`, containers resolved through Cloud Asset Inventory, granted by container-level IAM so a new project under a declared folder onboards with no change, with a per-project outcome snapshot so a denied project reads as a finding rather than an empty fleet. | `spec.scope`, Asset Inventory resolution, folder-level IAM inheritance, `fleet_scope.json`, single-project assumptions inventory, sequencing | Design of record; phases 1 to 3 mechanisms (explicit projects; folders and organisations; Shared VPC hosts and Metrics Scopes) implemented, and phase 1's IAM bindings, installer keys and chart render; phases 2 and 3's IAM and installer pieces, phase 1's MCP-server piece, and later phases pending |
...
| `deploy/index.md` | Site page | Hub for the deploy section: Docker, network policies, Minty, release versioning, telemetry, GitOps. | Navigation | — |
| `deploy/kustomize.md` | Site page | The gateway NetworkPolicy and the Service the operator renders for the Platform Agent, how to adjust their CIDRs, and the Kustomize dev copies under `k8s-operator/config/`. | Operator-rendered network objects | — |
...
| `terraform/modules/kube-agents-iam/README.md` | Component README | Reusable Terraform module for provisioning the agent's GSA, Workload Identity binding, read-only IAM role set, and the read grants in the projects its `scope` input names. | GCP IAM, Workload Identity, role grants | Infrastructure engineers |
