...
| `agents/platform/governance/*.md` | SOP playbook | Uniform cron-run governance playbooks, each with a purpose line and an execution checklist. The live ones are fleet audits: each emits a validated findings file and routes it through the `fleet-audit` skill, which publishes it as the stream's ledger issue. The rest are retained on disk but unscheduled — their watchdogs shipped disabled and were then retired from the cron roster. `inventory.md` is the first-run environment-discovery SOP, run by the bootstrap kanban card rather than by cron. | Fleet audits, drift reconciliation, cost, capacity, upgrades, first-run inventory | Runtime playbooks fired by the cron watchdogs (inventory: by the bootstrap scan card); the site's governance-sops page names them and says which are live |
