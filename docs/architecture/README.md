...
3. **Agents coordinate through durable, attributable state, or through authenticated synchronous
   RPC where a fix is already on the runbook.** The GitOps repo and the Operational Knowledge
   Framework (OKF) are the substrates for everything else. A valid, authenticated call from a peer
   agent is sufficient authorisation for runbook work; the caller's identity token is the record.
