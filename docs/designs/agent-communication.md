...
A `needs_input` block is sticky by design: nothing auto-recovers it, and only a human's `unblock` (or `archive`) ends it. Since #656 the `kanban-board-health` job on the Platform Agent's roster names any card blocked past `blocked_stale_hours` (24 h) in the home channel once a day, with its kind, reason and the commands to move it, until someone acts.
