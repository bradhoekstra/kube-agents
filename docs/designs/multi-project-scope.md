...
- **Two caps of 100, enforced in different places.** Each declared list (`projects`, `folders`,
  ...
  containers sorted by ID; a container that does not fit is skipped and the next one is still tried (its members are carried `over-cap` and do not count against the containers after it),
