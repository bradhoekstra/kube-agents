...
> This step gives the cluster an agent **immediately**. Even without it, the `cluster-agent-reconcile` cron automatically gives every cluster in every project in scope — including the management cluster where kube-agents runs — an agent on its next run; this skill just does it now.

## Stop managing

Because reconciliation manages **all** clusters in every project in scope, deleting a still-existing cluster's profile alone won't stick — the next reconcile recreates it. To stop managing a cluster:

- **Tear down the cluster** (see `gke-cluster-creation`); reconcile prunes its profile automatically.
- Or **exclude it**, as a `project/location/cluster` triple in `spec.scope.exclude.clusters`. Where the triple goes depends on who owns the CR:
  - **Installer-managed install** (the `PlatformAgent` carries Helm's `meta.helm.sh/release-name` annotation): the chart renders `spec.scope` from `install.env` on every full `upgrade.sh`, so the triple belongs in `SCOPE_EXCLUDE_CLUSTERS` there. You cannot edit that file from the pod: patch the CR for the immediate effect if the user wants it now, and tell them, in the report, the exact line to record (`SCOPE_EXCLUDE_CLUSTERS="<project>/<location>/<cluster>"`, space-separated with any existing entries) before their next full upgrade. A CR patch alone lasts until that upgrade, and do not promise a refusal: the installer refuses to drop a hand-set exclusion only while `install.env` records neither `SCOPE_EXCLUDE_*` key and `SCOPE_PROJECTS` names nothing or none of the CR's projects, so an install that already records an exclusion key or a shared project gets none, and the line has to be recorded.
  - **Hand-managed CR**: patch `spec.scope.exclude.clusters` on the `PlatformAgent`; the operator delivers it to the reconcile, which prunes the profile on its next run.
  - The older `RECONCILE_EXCLUDE` bare-name variable on the `cluster-agent-reconcile` cron still works for one release.
