...
- Or **exclude it**, as a `project/location/cluster` triple in `spec.scope.exclude.clusters`. Where the triple goes depends on who owns the CR:
  - **Installer-managed install** (the `PlatformAgent` carries Helm's `meta.helm.sh/release-name` annotation): the chart renders `spec.scope` from `install.env` on every full `upgrade.sh`, so the triple belongs in `SCOPE_EXCLUDE_CLUSTERS` there. You cannot edit that file from the pod: patch the CR for the immediate effect if the user wants it now, and tell them, in the report, the exact line to record (`SCOPE_EXCLUDE_CLUSTERS="<project>/<location>/<cluster>"`, space-separated with any existing entries) before their next full upgrade. A CR patch alone lasts until that upgrade; the installer refuses to drop a hand-set exclusion only while `SCOPE_PROJECTS` names nothing or names a project the CR does not carry.
