...
2. Run `setup` once from the editor and grant the permissions it asks for: Forms,
   ...
   It logs the share link and the edit link. Re-running it is safe; it reports the existing
   form rather than creating another.
...
The `external-feedback` label must exist on the repository; it does. GitHub silently drops
an unknown label from the create call rather than failing, so a missing label shows up as
issues without it, not as an error.
